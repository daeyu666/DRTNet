...
EXPERIMENTS = [
    ("01_drt_normal", "DRT + SAFA + CESR", False),
    ("02_drt_safa_single_scale", "DRT + single-scale SAFA + CESR", True),
    ("03_drt_no_contrast", "DRT + SAFA, no CESR", False),
]
...
def build_model(single_scale_safa=False):
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else run_root / "evaluation"
    ...
    for exp_name, description, single_scale_safa in EXPERIMENTS:
        ...
        model = build_model(single_scale_safa=single_scale_safa)
