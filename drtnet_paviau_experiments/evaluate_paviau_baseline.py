...
DESCRIPTION = "True baseline: no rectangular transformer, no multiresolution features, no contrastive learning"
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else run_root / "evaluation_baseline"
    output_dir.mkdir(parents=True, exist_ok=True)

    checkpoint = Path(args.checkpoint) if args.checkpoint else run_root / EXPERIMENT_NAME / "best_PaviaU_baseline.pkl"
    if not checkpoint.exists():
        raise FileNotFoundError("Missing checkpoint: {}".format(checkpoint))

    _, test_list = build_datasets(args.root, "PaviaU", args.image_size, args.n_select_bands, args.scale_ratio)
    test_ref, test_lr, test_hr = test_list
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = baseline("baseline", args.scale_ratio, args.n_select_bands, 103, "PaviaU")
