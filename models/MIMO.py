...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        self.pixel_unshuffle = nn.PixelUnshuffle(2)  # 初始化 PixelUnshuffle 操作
    def forward(self, x):
        x = self.body(x)  # 执行前向计算
        x = self.pixel_unshuffle(x)  # 通过实例化后的 PixelUnshuffle 来执行操作
        return x
        # return torch.nn.PixelUnshuffle.pixel_unshuffle(self.body(x), 2) # 11.05
...
class MIMO(nn.Module):
    def __init__(self,n_select_bands,n_bands):
        ...
        self.upSample = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False)  # 8 11.05
        self.Conv96_31 = nn.Conv2d(in_channels=96, out_channels=n_bands, kernel_size=3, padding=(1, 1))
        self.Conv192_31 = nn.Conv2d(in_channels=192, out_channels=n_bands, kernel_size=3, padding=(1, 1))
        ...
        self.Conv3_64 = nn.Conv2d(in_channels=n_select_bands, out_channels=48, kernel_size=3, padding=(1, 1))
        self.Conv31_64 = nn.Conv2d(in_channels=n_bands, out_channels=48, kernel_size=3, padding=(1, 1))

    def forward(self, x, y):  # hr, lr n_bands
        ...
        # print('Z1', Z1.shape)  # torch.Size([1, 244, 64, 64])
