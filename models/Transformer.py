...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.num_heads = heads # nun_heads = 8
        head_dim = dim // heads
        self.scale = qk_scale or head_dim ** -0.5  # 0.25

        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias) # Linear(in_features=128, out_features=384, bias=False)
        self.attn_drop = nn.Dropout(dropout_rate)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(dropout_rate)
        self.out = {}

    def forward(self, input):  # 接受两个输入,输入字典
        # print(x.shape) #1, 4, 128, 128, 128
        # x_ = input['x'].permute(0, 2, 3, 1).contiguous()
        # y_ = input['y'].permute(0, 2, 3, 1).contiguous()
        # # x = x.permute(0, 2, 3, 1).contiguous()
        ...
        a = x = input['x'] # 1,1024,64
        b = y = input['y'] #

        # x = np.squeeze(x, axis=0)
        # y = np.squeeze(y, axis=0)

        B, N, C = x.shape #1,1024,64
        qkv1 = (
            self.qkv(x)  # linear
                .reshape(B, N, 3, self.num_heads, C // self.num_heads)
                .permute(2, 0, 3, 1, 4)
