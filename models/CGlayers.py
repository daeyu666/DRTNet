...
class CG_Conv2d(nn.Conv2d):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True):
        super(CG_Conv2d, self).__init__(in_channels, out_channels, kernel_size, stride,
...
# 创建 Conv2d 模块的实例
conv_layer = CG_Conv2d(in_channels, out_channels, kernel_size,padding=0)

# 创建随机输入张量
input_tensor = torch.randn(batch_size, in_channels, height, width)
# print('GC input',input_tensor.shape)
# 通过卷积层获取输出
# output_tensor = conv_layer(input_tensor)

# 打印输出的形状
# print("GC Output shape:", output_tensor.shape)
